...
int main()
{
  ...
  // Matrix dimension.
  int m = 101;
  // The difference between Unew and Uold.
  double e = 10;
  ...
  {
    // Setup a Matrix with all 0s and Boundry Conditions
    #pragma omp for
    for (int x=0; x<m; x++)
    {
    ...
    // Calclate Unew from Uold until the difference is < 0.001
    while (e > 0.001)
    {
      #pragma omp for
      for (int x=1; x<(m-1); x++)
      ...
      // Swap Unew and Uold so that Uold = Unew
      #pragma omp for
      for (int x=1; x<(m-1); x++)
      {
        for (int y=1; y<(m-1); y++)
